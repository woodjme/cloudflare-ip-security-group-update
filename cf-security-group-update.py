...
def add_ipv4_rule(group, address, port):
    ...
    print("Added %s : %i to %s (%s) " %
          (address, port, group.group_id, group.group_name))
...
def add_ipv6_rule(group, address, port):
    """ Add the IP address/port to the security group """
    group.authorize_ingress(GroupName=group.group_name,
                            IpPermissions=[{
                                'IpProtocol':
                                "tcp",
                                'FromPort':
...
def update_s3_policies_policies(ip_addresses):
    ...
    if not "S3_CLOUDFLARE_SID" in os.environ:
        print(
            "Not configured 'S3_CLOUDFLARE_SID' variable, so will not check S3"
        )
        return

    if not "S3_BUCKET_IDS_LIST" in os.environ and not "S3_BUCKET_ID" in os.environ:
        raise Exception(
            "Missing S3 basic configuration 'S3_BUCKET_IDS_LIST' or 'S3_BUCKET_ID'."
        )
    ...
    s3_policy_tuple = map(get_aws_s3_bucket_policy,
                          os.environ['S3_BUCKET_IDS_LIST'].split(","))
    if not s3_policy_tuple:
        s3_policy_tuple = [
            get_aws_s3_bucket_policy(os.environ['S3_BUCKET_ID'])
        ]
    ...
    for s3_tuple in s3_policy_tuple:
        ...
        if not 'Statement' in policy:
            raise Exception("Problem reading policy of S3 Bucket '%s'" %
                            (s3_id))
        for statement in policy['Statement']:
            if not "Sid" in statement:
                raise Exception(
                    "Problem reading Sid inside Statement of S3 Bucket '%s'" %
                    (s3_id))
            if ((not sid == statement['Sid']) or (not "Condition" in statement)
                    or (not "IpAddress" in statement["Condition"]) or
...
def update_security_group_policies(ip_addresses):
    ...
    try:
        ...
        try:
            ...
            print(
                'Missing environment variables SECURITY_GROUP_IDS_LIST and SECURITY_GROUP_ID. Will not update security groups.'
            )
            return
    ...
    if (not ports) or (not security_groups):
        raise Exception(
            'At least one TCP port and one security group ID are required.')
    ...
    for security_group in security_groups:
        ...
        for port in ports:
            for rule in current_rules:
                ...
                if rule['IpProtocol'] == 'tcp' and rule[
                        'FromPort'] == port and rule['ToPort'] == port:
                    for ip_range in rule['IpRanges']:
                        if ip_range['CidrIp'] not in ip_addresses[
                                'ipv4_cidrs']:
                            delete_ipv4_rule(security_group,
                                             ip_range['CidrIp'], port)
        ...
        if get_update_ipv6():
            ...
            for ipv6_cidr in ip_addresses['ipv6_cidrs']:
                for port in ports:
                    if not check_ipv6_rule_exists(current_rules, ipv6_cidr,
                                                  port):
            ...
            for port in ports:
                for rule in current_rules:
                    if rule['IpProtocol'] == 'tcp' and rule[
                            'FromPort'] == port and rule['ToPort'] == port:
                        for ip_range in rule['Ipv6Ranges']:
                            if ip_range['CidrIpv6'] not in ip_addresses[
                                    'ipv6_cidrs']:
                                delete_ipv6_rule(security_group,
                                                 ip_range['CidrIpv6'], port)
        else:
